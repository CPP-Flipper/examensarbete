...
INSERT INTO site (scid, plats, foretag, fabrik) VALUES
  ...
  (3, 'obbola',      'SCA', 'pappersbruk'),
  (4, 'munksund',    'SCA', 'pappersbruk'),
  (5, 'bollstabruk', 'SCA', 'sågverk'),
  (6, 'gällö',       'SCA', 'sågverk'),
  (7, 'härnösand',   'SCA', 'pelletfabrik'),
  (8, 'tunadal',     'SCA', 'sågverk');


INSERT INTO sce (scid, start_time, end_time, rate) VALUES
  (1, 1714642425000, 1746178753000, NULL),
  (2, 1714642425000, 1746178753000, NULL),
  (3, 1714642425000, 1746178753000, NULL),
  (4, 1714642425000, 1746178753000, NULL),
  (5, 1714642425000, 1746178753000, NULL),
  (6, 1714642425000, 1746178753000, NULL),
  (7, 1714642425000, 1746178753000, NULL),
  (8, 1714642425000, 1746178753000, NULL);
...
INSERT INTO data_int (info_id, t_stamp, data, statusId)
SELECT
    ids[ 1 + floor(random()*cnt) ]                            AS info_id,
    (start_ms + (gs.i-1) * (span_ms/(n_rows-1)))::BIGINT      AS t_stamp,
...
INSERT INTO data_string (info_id, t_stamp, data, statusId)
SELECT
    ids[1 + floor(random()*cnt)]                                      AS info_id,
    (start_ms + (gs.i-1) * (span_ms/(n_rows-1)))::BIGINT              AS t_stamp,
...
INSERT INTO data_date (info_id, t_stamp, data, statusId)
SELECT
    ids[1 + floor(random()*cnt)]                                      AS info_id,
    (start_ms + (gs.i-1) * (span_ms/(n_rows-1)))::BIGINT              AS t_stamp,
    to_timestamp(                                                   
